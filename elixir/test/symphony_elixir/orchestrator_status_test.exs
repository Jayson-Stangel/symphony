defmodule SymphonyElixir.OrchestratorStatusTest do
  ...
  test "orchestrator lets a turn finish after crossing the reserve then blocks the next turn" do
    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_api_token: nil,
      ...
      {:codex_worker_update, issue_id,
       %{
         ...
         payload: %{
           ...
           "params" => %{
             "tokenUsage" => %{
               "total" => %{"inputTokens" => 65, "outputTokens" => 5, "totalTokens" => 70}
             }
           }
         },
         timestamp: DateTime.utc_now()
    ...
    assert state.running[issue_id].codex_total_tokens == 70
    refute Map.has_key?(state.blocked, issue_id)

    error =
      "codex live token reserve exhausted before turn 2: total_tokens=70 limit=100 remaining=30 reserve=40"

    assert {:error, ^error} =
             Orchestrator.preflight_codex_turn(orchestrator_name, issue_id, 2)
  ...
  test "orchestrator preflight fails closed before an initial turn when its reserve cannot fit" do
    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_api_token: nil,
      codex_live_max_total_tokens: 100,
      codex_live_turn_token_reserve: 120,
      codex_live_max_turns: 0
    )

    issue_id = "issue-live-token-reserve-config"
    orchestrator_name = Module.concat(__MODULE__, :InitialTokenReserveOrchestrator)
    {:ok, pid} = Orchestrator.start_link(name: orchestrator_name)
    ...
    running_entry = %{
      pid: self(),
      ref: make_ref(),
      identifier: "MT-RESERVE-CONFIG",
      issue: %Issue{id: issue_id, identifier: "MT-RESERVE-CONFIG", state: "In Progress"},
      worker_host: nil,
      workspace_path: "/tmp/MT-RESERVE-CONFIG",
      session_id: nil,
      turn_count: 0,
      codex_input_tokens: 0,
      codex_output_tokens: 0,
    ...
    assert {:error, "codex live token reserve exhausted before turn 1: total_tokens=0 limit=100 remaining=100 reserve=120"} =
             Orchestrator.preflight_codex_turn(orchestrator_name, issue_id, 1)

    assert %{turn_count: 0, codex_total_tokens: 0} = :sys.get_state(pid).blocked[issue_id]
  end
